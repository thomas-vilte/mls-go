...
func testSignWithLabel(t *testing.T, v cryptoBasicsVector) {
	...
	// Verify the vector's signature against the public key
	pubKey := NewOpenMlsSignaturePublicKey(pubBytes, ECDSA_SECP256R1_SHA256)
...
func testEncryptWithLabel(t *testing.T, cs CipherSuite, v cryptoBasicsVector) {
	...
	kemOutputBytes := mustDecodeHex(t, v.EncryptWithLabel.KEMOutput)
	ciphertextBytes := mustDecodeHex(t, v.EncryptWithLabel.Ciphertext)
	ctx := mustDecodeHex(t, v.EncryptWithLabel.Context)
	wantPlaintext := mustDecodeHex(t, v.EncryptWithLabel.Plaintext)

	ct := &HpkeCiphertext{KEMOutput: kemOutputBytes, Ciphertext: ciphertextBytes}
	plaintext, err := DecryptWithLabel(privBytes, v.EncryptWithLabel.Label, ctx, ct, cs)
	if err != nil {
		t.Errorf("DecryptWithLabel(%q): %v", v.EncryptWithLabel.Label, err)
		return
	}
	if !EqualCT(plaintext, wantPlaintext) {
		t.Errorf("DecryptWithLabel(%q):\n  got  %x\n  want %x", v.EncryptWithLabel.Label, plaintext, wantPlaintext)
	}
}
